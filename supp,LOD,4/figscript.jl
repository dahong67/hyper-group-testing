...
DESDIR = "../../../hyper-group-testing/supp,LOD,4/designs/"
...
with_theme(THEME) do
	...
	hyperstyle = (n,m,q) -> (;
		color=get(Dict(32=>dcolors[5],16=>dcolors[4],12=>dcolors[7]),m,:black),
	...
	plotsens!(ax_sens, hypersens[(n=96,m=8,q=1)];  dorfmanstyle...)
	...
	text!(ax_eff, dorfmanstr(96,8); color=dorfmanstyle.color,
		position=(87,3.5), offset=(0,0), align=(:right,:top))
	...
	ax_eff = Axis(fig[1,2][1,1]; title="n=384 individuals",
		limits=(nothing,(0,12*1.14)), yticks=[0,1,384/40,384/48,384/32])
	vspan!(ax_eff, SHADELIMS...; color=SHADECOLOR)
	...
	vspan!(ax_sens, SHADELIMS...; color=SHADECOLOR)
	plotsens!(ax_sens, hypersens[(n=384,m=16,q=1)]; dorfmanstyle...)
	...
	text!(ax_eff, dorfmanstr(384,16); color=dorfmanstyle.color,
		position=(73,6.0), offset=(0,0), align=(:right,:top))
	...
	# Fig c
	fig[2,1] = GridLayout()
	ax_eff = Axis(fig[2,1][1,1]; title="n=384 individuals",
		limits=(nothing,(0,32*1.02)), yticks=[1,384/32,384/16,384/12])
	vspan!(ax_eff, SHADELIMS...; color=SHADECOLOR)
	lines!(ax_eff, hypereff[(n=384,m=32,q=2)]; hyperstyle(384,32,2)...)
	...
	plotsens!(ax_sens, hypersens[(n=384,m=12,q=2)]; hyperstyle(384,12,2)...)
	plotsens!(ax_sens, indivsens;                   indivstyle...)
	...
	vspan!(ax_sens, SHADELIMS...; color=SHADECOLOR)
	plotsens!(ax_sens, hypersens[(n=384,m=12,q=3)]; hyperstyle(384,12,3)...)
	plotsens!(ax_sens, hypersens[(n=384,m=12,q=2)]; hyperstyle(384,12,2)...)
	plotsens!(ax_sens, hypersens[(n=384,m=12,q=1)]; hyperstyle(384,12,1)...)
	plotsens!(ax_sens, indivsens;                   indivstyle...)
	
	text!(ax_eff, hyperstr(384,12,3); color=hyperstyle(384,12,3).color,
		position=(54,28), offset=(0,0))
	text!(ax_eff, hyperstr(384,12,2); color=hyperstyle(384,12,2).color,
		position=(20,384/12), offset=(4,-8), align=(:left,:top))
	text!(ax_eff, hyperstr(384,12,1); color=hyperstyle(384,12,1).color,
		position=(20,24), offset=(4,-16), align=(:left,:top))
	text!(ax_eff, "Individual testing"; color=indivstyle.color,
		position=(20,1), offset=(4,2))
	...
	for (gl,sub) in zip(contents(fig.layout),'a':'z')
		...
		# y-axis
		ylims!(ax_sens, (0.7,1.0))
		ax_sens.yticks = 0.75:0.05:0.95
	...
	save("fig-s8.png", fig; px_per_unit=2)
	save("fig-s8.pdf", fig)
	fig
end
