...
let n = 6144
	pal = palette([RGB(0,0,0); PlotThemes.wong_palette])
	prange = 0.001:0.001:0.2
	pticks = 0.01:0.01:0.2
	plot(size=(800,400),dpi=200,legend=:bottomright,
		xlabel=L"\textrm{Prevalence } (\%)",
		ylabel=L"\textrm{tests/individual}: \mathrm{E}\;T/n",
		xticks=(pticks,string.(convert.(Int,pticks*100))),xlims=(0,maximum(pticks)),
		yticks=0:0.2:1,ylims=(0,1),
		palette=pal
	)
	
	# Problem 2
	plot!(prange,p->hyper_ub(p,n);label="Upper bound for HYPER method",linewidth=3)
	plot!(prange,prob2_lb;label="Lower bound for problem 2: two-stage testing with conservative decoder",linewidth=3,linestyle=:dash)
	
	# Problem 1
	plot!(prange,adap_ub;label="Upper bound for fully adaptive method",linewidth=3)
	plot!(prange,prob1_lb;label="Lower bound for problem 1: any number of stages, any decoder, etc.",linewidth=3,linestyle=:dash)
