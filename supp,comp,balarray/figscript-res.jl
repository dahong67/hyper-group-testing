...
DAYS, SELECTED, SAVENAME =
	IdentityRange(53:53), CartesianIndex.([(6,1); (8,2);;]), "fig-s17.png"
	# IdentityRange(80:80), CartesianIndex.([(4,1); (9,5);;]), "fig-s18.png"
...
with_theme() do
	...
	for ((figidx, grididx), figlbl) in zip(selidx, 'a':'z')
		...
		bars = map(collect(pairs(capsgrid[grididx]))) do (method,(effcap,bavg,des))
			methodstr =
				(;indiv="Indiv.",hyper="HYPER",balarray="Bal. arrays")[method]
			desstr = (;
				indiv = _   -> "",
				hyper = des -> des.q == 1 ?
