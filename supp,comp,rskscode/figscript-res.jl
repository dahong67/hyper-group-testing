...
DAYS, SELECTED, SAVENAME =
	IdentityRange(53:53), CartesianIndex.([(6,1); (8,2);;]), "fig-s19.png"
	# IdentityRange(80:80), CartesianIndex.([(4,1); (6,2);;]), "fig-s20.png"
...
capsgrid = map(Iterators.product(SWABS,TESTS)) do budget
	...
	# Restricted HYPER (optimized across designs)
	restdes = distinct([(;n,m,q) for (n,m,q,_) in keys(rskscodeeff)])
	resthypercap, resthyperdes = findmax(restdes) do des
		effcapacity(budget..., des.n, hypereff[des], hypersens[des])
	end
	resthyper = (resthypercap...,des=resthyperdes)
	
	# RS-KS code-based (optimized across designs)
	rskscodecap, rskscodedes = findmax(keys(rskscodeeff)) do des
		effcapacity(budget..., des.n, rskscodeeff[des], rskscodesens[des])
	end
	rskscode = (rskscodecap...,des=rskscodedes)
	
	return (;indiv, hyper, resthyper, rskscode)
end
...
LEGEND = (;
	...
	resthyper2 = colorant"mediumseagreen" => "Restricted HYPER (q=2)",
	resthyper3 = colorant"darkgreen"      => "Restricted HYPER (q=3)",
	rskscode22 = colorant"dodgerblue"     => "RS-KS code (q=2,f=2)",
	rskscode23 = colorant"darkorchid2"    => "RS-KS code (q=3,f=2)",
	rskscode33 = colorant"purple4"        => "RS-KS code (q=3,f=3)",
)
...
METHODCOLOR = (;
	...
	resthyper = des -> LEGEND[Symbol("resthyper$(des.q)")][1],
	rskscode  = des -> LEGEND[Symbol("rskscode$(des.f)$(des.q)")][1],
)
...
with_theme() do
	fig = Figure(;resolution=(1200,1275))
		...
		bars = map(collect(pairs(capsgrid[grididx]))) do (method,(effcap,bavg,des))
			...
			desstr = (;
				...
				hyper = des -> des.q == 1 ?
					...
					"n:$(des.n)\nm:$(des.m)\nq:$(des.q)",
				resthyper = des -> "n:$(des.n)\nm:$(des.m)\nq:$(des.q)",
				rskscode  = des -> "n:$(des.n)\nm:$(des.m)\nq:$(des.q)\nf:$(des.f)",
			)[method](des)
			...
			bstr = (;
				...
				resthyper = des -> "b:$(intround(bavg))",
				rskscode  = des -> "b:$(intround(bavg))",
			)[method](des)
	...
	## Legend
	LAYOUT = [:indiv, :hyper1, :hyper2, :hyper3,
		:resthyper2, :resthyper3, :rskscode22, :rskscode23, :rskscode33]
	LEG = [LEGEND[m][1]=>rstrip(LEGEND[m][2],'*') for m in LAYOUT]
	Legend(row1[1,3], [PolyElement(;color) for (color,_) in LEG], last.(vec(LEG)),
		"Methods"; titlefont=assetpath("fonts", "NotoSans-Bold.ttf"),
		tellheight=false, tellwidth=true, framevisible=false,
		titlegap=28f0, rowgap=24f0)
