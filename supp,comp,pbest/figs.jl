...
md"""
# Figure: comparison of P-BEST with additional HYPER designs
"""
...
pbesteff = only(loadeff("../simulation/designs/pbest/"))

# ╔═╡ 744ec278-2216-11eb-2aee-6138f60f7722
pbestsens = only(loadsens("../simulation/designs/pbest/"))
...
let compfigs = [384=>[(n=384,m=32,q=2),(n=384,m=48,q=2),(n=384,m=convert(Int,2/(6/48)),q=2)]]
	# Plot colors
	...
	plot!(effplts[1],ylims=(0,24*1.14),yticks=[384/16,384/48,384/32])
	annotate!(effplts[1],[(18,-0.4,text("0",7)),(18,1.4,text("1",7))])
	...
	annotate!(effplts[1],[
		(21.75, 12.2, text(hypername(last(compfigs[1])[1]), 8, :left, :bottom, hypercolor[last(compfigs[1])[1].m])),
		(21.75,  8.2, text(hypername(last(compfigs[1])[2]), 8, :left, :bottom, hypercolor[last(compfigs[1])[2].m])),
		(21.75, 24.2, text(hypername(last(compfigs[1])[3]), 8, :left, :bottom, hypercolor[last(compfigs[1])[3].m])),
		(21.75,  7.8, text("P-BEST",                     8, :left, :top,    pbestcolor)),
		(21.75,  1.2, text("Individual testing",            8, :left, :bottom, indivcolor))
	])
	
	plot!.(effplts, bottom_margin=0mm,top_margin=-1.0mm)
	plot!.(sensplts,bottom_margin=0mm,top_margin=-2.5mm)
	plot!.([effplts; sensplts],right_margin=-1mm)
	
	plot(effplts...,sensplts...,layout=(2,1),link=:x,size=(450,400))
