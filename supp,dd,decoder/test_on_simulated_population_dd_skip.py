...
def decode_dd_skip(y,A):
	x_hat = (A.sum(0) == A.T.dot(y))            # possible defectives (PD)
	pd = np.nonzero(x_hat)[0]
	postests = np.nonzero(y == 1)[0]            # positive tests
	x_hat_dd = np.zeros((A.shape[1]))           # definite defectives (DD)
	for test in postests:
		pd_test = pd[A[test,pd] == 1]             # PDs in test
		if len(pd_test) == 1:
			x_hat_dd[pd_test[0]] = 1
	return x_hat,x_hat_dd
...
def run_test(A,ViralLoad,InfectionTime,OnsetTime,LOD,err_tol,fp_rate,itr=200000,pos_tol=2500):
	...
	for __ in range(itr):
		...
		x_hat,x_hat_dd = decode_dd_skip(y,A)
		x_hat_nondd = np.logical_and(x_hat == 1, x_hat_dd == 0)
		t = x_hat_nondd.sum() # non-DD putative positives
		T.append(t)
		if x.sum() > 0:
			viralload = ViralLoad[idx_n]
			x_hat_nondd = x_hat_nondd*(viralload > LOD) # samples that are putative positive and above the LOD
			x_hat = np.logical_or(x_hat_nondd == 1, x_hat_dd == 1)
...
if __name__ == '__main__':
	...
	Specificity_combined = np.zeros(ViralLoad.shape[1])
	Recall_n, Vl, Vi, Vo = [],[],[],[]
	...
	for t in range(args.start_time,args.end_time+1):
		...
		print('Time: %d; Case frequency: %.5f; Efficiency: %.2f; Sensitivity: %.4f; Specificity: %.4f; Fraction >LOD: %.4f' % (t,np.average(viralload > 0),ea,rec,spec, np.average(viralload[viralload > 0] > args.LOD)))
	...
	np.save('%s/Specificity_combined.n-%d_m-%d_q-%d.npy' % (args.savepath,args.n_individuals,args.m_pools,args.q_split),Specificity_combined)
